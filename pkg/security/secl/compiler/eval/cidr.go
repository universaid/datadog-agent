...
func (cm *CIDRMatcher) Matches(value IPMatcher) bool {
	...
	case CIDRValueType:
		...
		if ok {
			return cidrMatcher.IPMatches(cm.net.IP) || cidrMatcher.IPMatches(cm.getLastIP()) || cm.IPMatches(cidrMatcher.net.IP) || cm.IPMatches(cidrMatcher.getLastIP())
		}
		return false
	default:
		return false
...
func (cm *CIDRMatcher) getLastIP() net.IP {
	if cm.net.IP == nil {
		// the last IP is the network address OR NOT the mask address
		prefixLen, bits := cm.net.Mask.Size()
		if prefixLen == bits {
			// Make sure that our two slices are distinct, since they
			// would be in all other cases.
			lastIP := make([]byte, len(cm.net.IP))
			copy(lastIP, cm.net.IP)
			cm.lastIP = lastIP
		} else {
			firstIPInt, ipLen, _ := IPToInt(cm.net.IP)
			hostLen := uint(bits) - uint(prefixLen)
			lastIPInt := big.NewInt(1)
			lastIPInt.Lsh(lastIPInt, hostLen)
			lastIPInt.Sub(lastIPInt, big.NewInt(1))
			lastIPInt.Or(lastIPInt, firstIPInt)
			cm.lastIP = IntToIP(lastIPInt, ipLen)
		}
	}
	return cm.lastIP
}
